...
const fetchAllData = async () => {
 ...
 const contractDebtData = useMemo(() => {
   ...
   // Проверяем новый формат данных для auto_shipped_by_warehouse
   if (Array.isArray(notShippedData)) {
     // Проверяем, это новый формат со складами или старый
     if (notShippedData.length > 0 && notShippedData[0].name && notShippedData[0].models) {
       // Новый формат - массив складов
       notShippedCount = notShippedData.reduce((total, warehouse) => {
         if (!warehouse.models || !Array.isArray(warehouse.models)) return total;
         
         const warehouseCount = warehouse.models.reduce((sum, model) => {
           if (selectedModel && model.model !== selectedModel) return sum;
           return sum + parseInt(model.sold || 0);
         }, 0);
         
         return total + warehouseCount;
       }, 0);
     } else {
 ...
 const getRegionData = (status, modelId) => {
   if (status === 'inTransit') {
     ...
     // Для не отгруженных автомобилей - новый формат со складами
     if (Array.isArray(notShippedData) && notShippedData.length > 0 && notShippedData[0].name && notShippedData[0].models) {
       // Новый формат - склады
       return notShippedData
         .map(warehouse => {
           let totalCount = 0;
           ...
           return {
             name: warehouse.name,
             value: totalCount,
             isWarehouse: true // Маркер для определения типа
           };
         })
         .filter(warehouse => warehouse.value > 0)
         .sort((a, b) => b.value - a.value);
     } else {
       // Старый формат или другие данные
...
const getDealerData = (status, regionName, selectedModelId) => {
  if (status === 'inTransit' && Array.isArray(inMovementData)) {
    ...
    // Для не отгруженных автомобилей - новый формат
    if (Array.isArray(notShippedData) && notShippedData.length > 0 && notShippedData[0].name && notShippedData[0].models) {
      // Новый формат - ищем склад
      const warehouse = notShippedData.find(w => w.name === regionName);
      
      if (warehouse && warehouse.models && Array.isArray(warehouse.models)) {
        // Группируем модели по названию
        const modelGroups = {};
        
        warehouse.models.forEach(model => {
          if (!selectedModelId || model.model === selectedModelId) {
            if (!modelGroups[model.model]) {
              ...
              // Если группа уже существует, просто увеличиваем счетчик
              modelGroups[model.model].value += parseInt(model.sold || 0);
              // Добавляем дополнительную запись модели, если это другой экземпляр
              modelGroups[model.model].models.push({
                id: model.model,
                name: carModelMap[model.model]?.name || model.model,
                count: parseInt(model.sold || 0),
                img: model.photo_sha 
    ...
    } else if (Array.isArray(notShippedData)) {
      // Старый формат
 ...
 const isWarehouseData = () => {
   return Array.isArray(notShippedData) && 
          notShippedData.length > 0 && 
          notShippedData[0].name && 
          notShippedData[0].models;
 };
 ...
 const warehouseData = useMemo(() => {
   ...
   return notShippedData.map(warehouse => {
 ...
 const renderSidebarContent = () => {
   ...
   // Уровень 1: Список регионов или складов
   if (activeDetailLevel === 1) {
     // Получаем данные по регионам
     let regions = [];
     const isWarehouse = selectedStatus === 'notShipped' && isWarehouseData();
     
     if (sourceData && Array.isArray(sourceData)) {
       if (isWarehouse) {
         // Обработка складов
         regions = sourceData.map(warehouse => {
           let value = 0;
       ...
       <div className="h-full flex flex-col">
         ...
         <div className={`flex items-center gap-3 p-4 pt-14 border-b ${isDark ? 'border-gray-700 bg-gray-800/90' : 'border-gray-200 bg-white'}`}>
           ...
           <div>
             ...
             <div className={`text-sm ${isDark ? 'text-gray-400' : 'text-gray-600'}`}>
               {isWarehouse ? 'Всего складов' : t('sidebar.totalRegions')}: {regions.length}
             </div>
           </div>
         </div>
         ...
         <div className="flex-1 overflow-y-auto p-3">
           <div className={`text-sm ${isDark ? 'text-gray-400' : 'text-gray-600'} mb-3 px-2 flex items-center gap-2`}>
             {isWarehouse ? <Archive size={14} /> : <MapPin size={14} />}
             <span>{isWarehouse ? 'Выберите склад:' : t('sidebar.selectRegion') + ':'}</span>
           </div>
               ...
               {regions.map((region, index) => (
                 ...
                 >
                   <div className="flex justify-between items-center">
                     <div className="flex items-center gap-2">
                       {isWarehouse ? <Archive size={18} className={`text-${statusColor}-400`} /> : <MapPin size={18} className={`text-${statusColor}-400`} />}
   ...
   // Уровень 2: Список дилеров для выбранного региона или модели для склада
   if (activeDetailLevel === 2 && selectedRegion) {
     ...
     if (sourceData && Array.isArray(sourceData)) {
       if (isWarehouse) {
         // Для складов показываем модели
         const warehouse = sourceData.find(w => w.name === selectedRegion);
         ...
         if (warehouse && warehouse.models && Array.isArray(warehouse.models)) {
           ...
           // Преобразуем в массив и сортируем
           dealers = Object.values(modelGroups)
             .filter(group => group.value > 0)
             .sort((a, b) => b.value - a.value);
         }
       } else {
     ...
     return (
       <div className="h-full flex flex-col">
         ...
         <div className={`flex items-center gap-3 p-4 pt-14 border-b ${isDark ? 'border-gray-700 bg-gray-800/90' : 'border-gray-200 bg-white'}`}>
           ...
           <div>
             <h3 className={`text-lg font-medium ${isDark ? 'text-white' : 'text-gray-900'} flex items-center gap-1.5`}>
               {isWarehouse ? <Archive size={16} className={`text-${statusColor}-400`} /> : <MapPin size={16} className={`text-${statusColor}-400`} />}
           ...
           <div className={`ml-auto px-2.5 py-1 rounded-md text-sm ${
             ...
             <span className={isDark ? 'text-gray-400' : 'text-gray-600'}>
               {isWarehouse ? 'Модели:' : t('sidebar.dealers') + ':'}
             </span>
         ...
         <div className="flex-1 overflow-y-auto p-3">
           <div className={`text-sm ${isDark ? 'text-gray-400' : 'text-gray-600'} mb-3 px-2 flex items-center gap-2`}>
             {isWarehouse ? <Car size={14} /> : <Users size={14} />}
             <span>{isWarehouse ? 'Модели на складе:' : t('sidebar.dealersList') + ':'}</span>
           </div>
...
<div className="space-y-2.5">
  {dealers.map((dealer, index) => (
    <div 
      ...
      onClick={() => isWarehouse ? null : handleDealerSelect(dealer.name)}
    >
      {isWarehouse ? (
        // Специальное отображение для моделей на складе
        <div className="flex items-center gap-3">
          <div className={`w-16 h-16 rounded-lg overflow-hidden flex-shrink-0 ${
            isDark ? 'bg-gray-700' : 'bg-gray-200'
          }`}>
            <img 
              src={dealer.models[0]?.img || carModelMap[dealer.name]?.img || ''}
              alt={dealer.name} 
              className="w-full h-full object-contain"
              onError={(e) => {
                e.target.onerror = null;
          ...
          <div className="flex-1">
            ...
            <div className={`text-sm ${isDark ? 'text-gray-400' : 'text-gray-600'} mt-1`}>
              На складе: {dealer.value} {t('dealer.units')}
            </div>
          </div>
          <div className={`px-3 py-2 rounded-lg bg-${statusColor}-900/40 text-${statusColor}-300 flex flex-col items-center border border-${statusColor}-800/30`}>
            <span className="text-lg font-bold">{dealer.value}</span>
            <span className="text-xs">{t('dealer.units')}</span>
          </div>
        </div>
      ) : (
        // Обычное отображение для дилеров
        <div className="flex justify-between items-center">
  ...
  <div className={`${isDark ? 'bg-gradient-to-br from-gray-900 to-gray-950' : 'bg-gray-50'} p-4 font-sans ${isDark ? 'text-gray-300' : 'text-gray-700'} min-h-screen relative`}>
    ...
    <div className="">
     ...
     <div className="grid grid-cols-1 lg:grid-cols-1 gap-5 mb-5">
       ...
       {/* НОВЫЙ БЛОК - СКЛАДЫ */}
       {isWarehouseData() && warehouseData.length > 0 && (
         <div className={`${isDark ? 'bg-gray-800/70' : 'bg-white'} backdrop-blur-sm rounded-lg border ${isDark ? 'border-gray-700/50' : 'border-gray-200'} shadow-md overflow-hidden`}>
           <div className={`flex justify-between items-center p-3 border-b ${isDark ? 'border-gray-700' : 'border-gray-200'}`}>
             <h3 className={`text-base font-medium ${isDark ? 'text-white' : 'text-gray-900'} flex items-center gap-2`}>
             ...
             <div className="text-[16px] text-blue-300">
               Складов: <span className="font-bold">{warehouseData.length}</span>
             </div>
           </div>
