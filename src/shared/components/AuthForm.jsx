...
export default function AuthForm() {
  ...
  // Функция для получения списка филиалов
  const fetchBranches = async () => {
    setIsLoadingBranches(true);
    try {
      const token = localStorage.getItem('authToken');
      const response = await fetch('https://bankapi.uzautomotors.com/inventory/proxy', {
        method: 'POST',
        headers: {
        ...
        body: JSON.stringify({
          url: '/b/ap/inventory_list&get_branches' // или другой эндпоинт для получения филиалов
        })
      });

      if (response.ok) {
        const data = await response.json();
        // Предполагаем, что API возвращает массив филиалов
        setBranches(data || [
          { 
            id: '1', 
            name: 'Филиал Ташкент',
            city: 'Ташкент',
            address: 'ул. Амира Темура, 109',
            employees: 45,
            activeDeals: 127
          },
          { 
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      if (success) {
        // После успешной авторизации показываем выбор филиала
  ...
  return (
    <div className={`flex flex-col items-center justify-center min-h-screen overflow-hidden relative ${isDark ? 'dark' : ''}`}>
      ...
      {/* Основной контент */}
      <div className={`w-full ${showBranchSelector ? 'max-w-4xl' : 'max-w-md'} px-4 z-10 transition-all duration-500`}>
        <AnimatePresence mode="wait">
          {!showBranchSelector ? (
            ...
            >
              ...
              <p className={`text-center ${isDark ? 'text-gray-500' : 'text-gray-400'} text-xs mt-6`}>
                © 2025 CarSale Analytics • Защищено шифрованием
              </p>
            </motion.div>
          ) : (
            /* Форма выбора филиала */
            <motion.div
              key="branch-selector"
              className={`${
                isDark 
            ...
            >
              ...
              <div className="text-center mb-8">
                ...
                <p className={`${
                  ...
                  Выберите филиал для работы в системе
                </p>
              </div>

              {/* Список филиалов карточками */}
              <div className="space-y-4 max-h-[400px] overflow-y-auto px-1">
                {isLoadingBranches ? (
                  <div className="flex justify-center items-center py-8">
                    <svg className="animate-spin h-8 w-8 text-purple-600" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                      <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"></circle>
                      <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
                    </svg>
                  </div>
                      ...
                      <div className="flex items-start space-x-4">
                        ...
                        <div className={`w-12 h-12 rounded-xl ${
                          ...
                          <svg width="24" height="24" viewBox="0 0 24 24" fill="none">
                            <path d="M3 21H21" stroke={isDark ? "#a78bfa" : "#8b5cf6"} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
                            <path d="M3 7V21" stroke={isDark ? "#a78bfa" : "#8b5cf6"} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
                            <path d="M21 7V21" stroke={isDark ? "#a78bfa" : "#8b5cf6"} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
                            <path d="M3 7L12 2L21 7" stroke={isDark ? "#a78bfa" : "#8b5cf6"} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
                            <path d="M9 21V13H15V21" stroke={isDark ? "#a78bfa" : "#8b5cf6"} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
                          </svg>
                        </div>
                        ...
                        <div className="flex-1">
                          ...
                          <div className="flex items-center space-x-4 mt-3">
                            ...
                            <div className="flex items-center space-x-1">
                              <svg width="16" height="16" viewBox="0 0 24 24" fill="none">
                                <path d="M8 12H16" stroke={isDark ? "#9ca3af" : "#6b7280"} strokeWidth="1.5" strokeLinecap="round"/>
                                <path d="M12 16V8" stroke={isDark ? "#9ca3af" : "#6b7280"} strokeWidth="1.5" strokeLinecap="round"/>
                              ...
                              <span className={`text-xs ${
                                ...
                                {branch.activeDeals} активных сделок
                              </span>
                            </div>
                          </div>
                        </div>
